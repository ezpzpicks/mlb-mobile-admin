"""Leakage-safe player-strength x slot-matchup interaction test.

Tests whether elite players within a depth-chart slot should be less sensitive to
opponent slot matchup than weaker players. Uses the existing RB/WR pregame
research dataset, reconstructs RB1/RB2 and WR1/WR2/WR3 from lagged opportunity,
learns defense-slot residuals from PRIOR games only, and keeps 2025 untouched as
the final holdout.
"""
...
def _add_market_strength_and_slot_matchup(
    ...
    *,
    ...
    df = df[df[player_only_baseline_col].gt(0.5) & df[actual_col].notna()].copy()

    # Pregame strength is relative to players occupying the SAME slot that week.
    # All inputs are lagged, so this ranking is leakage-safe.
    df["strength_pct"] = (
        df.groupby(["season", "week", "slot"])[player_only_baseline_col]
        .rank(pct=True, method="average")
        .clip(0.0, 1.0)
    )
    df["strength_centered"] = df["strength_pct"] - 0.5
    df["tier"] = pd.cut(
        df["strength_pct"],
    ...
    # Normalize each opponent performance by the player's own pregame baseline.
    # A defense allowing 80 yards to an elite WR1 is not treated the same as
    # allowing 80 to a weak WR1.
...
def main() -> None:
    out = Path("artifacts/nfl_player_strength_interaction")
    ...
    results: dict[str, Any] = {
        "research_version": "nfl-player-strength-slot-matchup-interaction-2026-09-24",
        "method": (
            "Slots reconstructed from lagged pregame workload; strength percentile computed within "
            "slot/week from player-only pregame expected yards; defense-slot matchup learned from "
    ...
    path = out / "nfl_player_strength_interaction_results.json"
