...
MARGIN_FEATURES = [
    "point_diff_strength_diff", "pass_matchup_diff",
...
def build_dataset() -> pd.DataFrame:
    ...
    for season in SEASONS:
        ...
        for _, game in games.iterrows():
            ...
            rest_edge = float(nflb.clamp((home_rest - away_rest) * 0.18, -1.5, 1.5))
            hfa_points = num(nflb._automatic_home_field_advantage(season, week, home_team, game).get("value"), 0.0)
            weather_value = weather_adjustment(game)
            baseline = nflb._project_matchup(
                away, home,
                {"offense_absence": 0.0, "defense_absence": 0.0},
                {"offense_absence": 0.0, "defense_absence": 0.0},
                {
                    "home_field": hfa_points, "home_rest_edge": rest_edge,
                    "weather_total_adjustment": weather_value,
                    "manual_total_adjustment": 0.0, "manual_home_margin_adjustment": 0.0,
                },
            )
            row = {
                ...
                "actual_margin_no_hfa": num(game["Home Score"]) - num(game["Away Score"]) - hfa_points,
                "actual_total": num(game["Home Score"]) + num(game["Away Score"]),
                "hfa_points": hfa_points,
                "existing_margin": num(baseline.get("margin")),
                "existing_total": num(baseline.get("total")),
                ...
                "weather_adjustment": weather_value,
            }
            rows.append(row)
...
def target_result(discovery: pd.DataFrame, confirmation: pd.DataFrame, holdout: pd.DataFrame, target: str, candidates: list[str], forced: set[str] | None = None):
    ...
    unstable = [
        ...
        if feature not in forced and np.sign(dm.params.get(feature, 0.0)) != np.sign(cm.params.get(feature, 0.0))
    ]
    if unstable and len(selected) - len(unstable) >= 1:
        ...
        selected = [feature for feature in selected if feature not in unstable]
        dm = fit(discovery, target, selected)
        cm = fit(confirmation, target, selected)
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--output-dir", default="artifacts/nfl_game_regression_v2"); args = parser.parse_args()
    ...
    margin = target_result(discovery, confirmation, holdout, "actual_margin_no_hfa", MARGIN_FEATURES)
    total = target_result(discovery, confirmation, holdout, "actual_total", TOTAL_FEATURES)
    margin_full_pred = margin["pred"] + holdout["hfa_points"].to_numpy(float)
    mr = holdout["actual_margin"].to_numpy(float) - margin_full_pred; tr = holdout["actual_total"].to_numpy(float) - total["pred"]
    margin["holdout_metrics_full_margin"] = metrics(holdout["actual_margin"], margin_full_pred)
    baseline_margin_metrics = metrics(holdout["actual_margin"], holdout["existing_margin"].to_numpy(float))
    baseline_total_metrics = metrics(holdout["actual_total"], holdout["existing_total"].to_numpy(float))
    results = {
        "research_version": "nfl-game-regression-v2-2026-08-20",
        "method": "Production nflverse metrics; remove exact collinearity; rolling HFA treated as a pregame offset; 2021-23 HC3 OLS backward selection p<0.05; 2024 sign confirmation; final refit 2021-24; 2025 out-of-sample evaluation; no market predictors",
        ...
        "edge_accuracy": edge_accuracy(holdout, margin_full_pred, total["pred"]),
        "existing_model_baseline": {
            "margin_metrics": baseline_margin_metrics, "total_metrics": baseline_total_metrics,
            "edge_accuracy": edge_accuracy(holdout, holdout["existing_margin"].to_numpy(float), holdout["existing_total"].to_numpy(float)),
        },
    }
    clean = strip_predictions(results); (output / "nfl_game_regression_results_v2.json").write_text(json.dumps(clean, indent=2, sort_keys=True))
    generated = f'''"""Generated by research/nfl_game_regression_v2.py. Do not hand edit."""\nMODEL_RESEARCH_VERSION = {clean["research_version"]!r}\nMARGIN_INTERCEPT = {clean["margin"]["intercept"]!r}\nMARGIN_COEFFICIENTS = {clean["margin"]["coefficients"]!r}\nTOTAL_INTERCEPT = {clean["total"]["intercept"]!r}\nTOTAL_COEFFICIENTS = {clean["total"]["coefficients"]!r}\nMARGIN_RESIDUAL_SD = {clean["residual_distribution"]["margin_sd"]!r}\nTOTAL_RESIDUAL_SD = {clean["residual_distribution"]["total_sd"]!r}\nMARGIN_TOTAL_RESIDUAL_CORRELATION = {clean["residual_distribution"]["correlation"]!r}\nHOLDOUT_SUMMARY = {clean["edge_accuracy"]!r}\n'''
    (output / "nfl_game_regression_generated_v2.py").write_text(generated)
