...
SLATE_COLUMNS = [
    ...
    # Optional public-summary columns. If your sheet/app has any of these,
    # the Full Slate Summary tab will use them automatically.
    "Better ML %", "ML Model %", "Moneyline %", "Better Moneyline %",
    "Away ML %", "Home ML %"
]
...
.summary-card{border:1px solid rgba(56,189,248,.20);border-radius:1.05rem;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.78));box-shadow:0 14px 36px rgba(0,0,0,.22);overflow:hidden;margin-bottom:1rem}.summary-head{display:flex;align-items:flex-start;justify-content:space-between;gap:.65rem;padding:.68rem .78rem;border-bottom:1px solid rgba(148,163,184,.14);background:linear-gradient(135deg,rgba(37,99,235,.16),rgba(14,165,233,.06))}.summary-title{font-size:.98rem;font-weight:950;color:#fff;letter-spacing:-.02em}.summary-sub{font-size:.67rem;color:#94a3b8;font-weight:800;margin-top:.08rem}.summary-count{padding:.22rem .46rem;border-radius:999px;color:#dbeafe;background:rgba(37,99,235,.14);border:1px solid rgba(96,165,250,.20);font-size:.66rem;font-weight:950;white-space:nowrap}.summary-scroll{overflow-x:auto}.slate-summary-table{width:100%;border-collapse:collapse;min-width:690px}.slate-summary-table th{background:rgba(15,23,42,.96);color:#93c5fd;text-align:left;text-transform:uppercase;letter-spacing:.035em;font-size:.58rem;font-weight:950;padding:.42rem .48rem;border-bottom:1px solid rgba(148,163,184,.16);white-space:nowrap}.slate-summary-table td{padding:.42rem .48rem;border-bottom:1px solid rgba(148,163,184,.08);color:#e5e7eb;font-size:.68rem;vertical-align:middle;line-height:1.18}.slate-summary-table tr:last-child td{border-bottom:none}.slate-summary-table tr:nth-child(even) td{background:rgba(30,41,59,.18)}.summary-game{font-weight:950;color:#fff;line-height:1.14;max-width:135px}.summary-plain{color:#e5e7eb;font-weight:850}.summary-muted{display:block;color:#94a3b8;font-size:.58rem;margin-top:.07rem;line-height:1.12}.summary-green{display:inline-flex;align-items:center;border-radius:999px;padding:.15rem .36rem;background:rgba(34,197,94,.14);border:1px solid rgba(34,197,94,.34);color:#bbf7d0;font-size:.61rem;font-weight:950;white-space:nowrap}.summary-cell-green{border-left:3px solid rgba(34,197,94,.72);background:rgba(34,197,94,.06)!important}.summary-k{font-weight:850;color:#f8fafc}.screenshot-note{color:#94a3b8;font-size:.68rem;line-height:1.25;margin:.1rem 0 .55rem}
/* Premium centered EZPZ tab navigation */
...
def render_hero():
    ...
    last_updated = datetime.now(ZoneInfo('America/New_York')).strftime('%b %d, %Y %I:%M %p ET')
    st.markdown(f"""
...
def first_existing_value(row, columns, default=""):
    for col in columns:
...
def extract_percentage_text(value):
    text = safe_text(value)
    if not text:
        return ""
...
def get_better_ml_percent(row):
    direct = first_existing_value(row, ["Better ML %", "ML Model %", "Moneyline %", "Better Moneyline %"])
    if direct:
        return direct if "%" in direct else f"{direct}%"
    better_ml = safe_text(row.get("Better ML", ""))
    parsed = extract_percentage_text(better_ml)
    if parsed:
        return parsed

    better_team = safe_text(row.get("Better ML", ""))
    away = safe_text(row.get("Away Team", ""))
    home = safe_text(row.get("Home Team", ""))
    away_pct = first_existing_value(row, ["Away ML %"])
    home_pct = first_existing_value(row, ["Home ML %"])
    if better_team and away and better_team.lower() == away.lower() and away_pct:
        return away_pct if "%" in away_pct else f"{away_pct}%"
    if better_team and home and better_team.lower() == home.lower() and home_pct:
        return home_pct if "%" in home_pct else f"{home_pct}%"
    return "—"


def summary_green_tag(text):
    return f"<span class='summary-green'>{esc(text)}</span>"


def summary_plain(text, muted=""):
    muted_html = f"<span class='summary-muted'>{esc(muted)}</span>" if safe_text(muted) else ""
    return f"<span class='summary-plain'>{esc(text)}</span>{muted_html}"


def summarize_k_for_table(value, green_bet_types=None):
    green_bet_types = green_bet_types or set()
    text = safe_text(value)
    if not text or text.lower().startswith("no "):
        return "—", False
    details = extract_k_details(text)
    pitcher = details.get("pitcher", "") or "Pitcher"
    projection = details.get("projection", "")
    bet_key = normalize_bet_type_text(text)
    bet_type = display_bet_type(details.get("bet_type") or bet_key or "")
    line_value = details.get("line", "")
    top = pitcher
    if projection:
        top += f" · {projection}K"
    sub_parts = []
    if bet_type:
        sub_parts.append(bet_type)
    if line_value:
        sub_parts.append(f"L {line_value}")
    is_green = bool(bet_key and bet_key in green_bet_types)
    if is_green:
        return summary_green_tag(top) + (f"<span class='summary-muted'>{esc(' · '.join(sub_parts))}</span>" if sub_parts else ""), True
    return summary_plain(top, " · ".join(sub_parts)), False


def render_full_slate_summary(today_slate, green_bet_types=None):
    green_bet_types = green_bet_types or set()
    st.markdown('<div class="section-title">Full Slate Snapshot</div>', unsafe_allow_html=True)
    st.markdown('<div class="screenshot-note">Compact view for screenshots. Green bubbles only appear for bet types that are currently winning in your tracked record.</div>', unsafe_allow_html=True)
    ...
    for _, row in today_slate.iterrows():
        ...
        ml_pct = get_better_ml_percent(row)
        nrfi_raw = safe_text(row.get("NRFI Grade", ""))
        nrfi = display_bet_type(nrfi_raw) or "—"

        ml_is_green = is_green_play(ml_grade_raw, green_bet_types)
        nrfi_is_green = is_green_play(nrfi_raw, green_bet_types)
        away_k, away_green = summarize_k_for_table(row.get("Away Pitcher K + Grade", ""), green_bet_types)
        home_k, home_green = summarize_k_for_table(row.get("Home Pitcher K + Grade", ""), green_bet_types)

        ml_sub = " · ".join([x for x in [ml_grade, ml_odds] if x])
        ml_cell = (summary_green_tag(better_ml) if ml_is_green else summary_plain(better_ml))
        if ml_sub:
            ml_cell += f"<span class='summary-muted'>{esc(ml_sub)}</span>"

        nrfi_cell = summary_green_tag(nrfi) if nrfi_is_green else summary_plain(nrfi)
        ml_td_class = " class='summary-cell-green'" if ml_is_green else ""
        away_td_class = " class='summary-cell-green'" if away_green else ""
        home_td_class = " class='summary-cell-green'" if home_green else ""
        nrfi_td_class = " class='summary-cell-green'" if nrfi_is_green else ""

        rows_html.append(f"""
        <tr>
          <td><div class="summary-game">{esc(game)}</div></td>
          <td{ml_td_class}>{ml_cell}</td>
          <td><span class="summary-plain">{esc(ml_pct)}</span></td>
          <td{away_td_class}><span class="summary-k">{away_k}</span></td>
          <td{home_td_class}><span class="summary-k">{home_k}</span></td>
          <td{nrfi_td_class}>{nrfi_cell}</td>
        </tr>
        """)
    ...
    <div class="summary-card">
      <div class="summary-head">
        <div><div class="summary-title">EZPZ Full Slate Snapshot</div><div class="summary-sub">Today&apos;s saved MLB board</div></div>
        <div class="summary-count">{len(today_slate)} Games</div>
      </div>
      <div class="summary-scroll">
        <table class="slate-summary-table">
          <thead><tr><th>Game</th><th>Better ML</th><th>ML %</th><th>Away K</th><th>Home K</th><th>NRFI/YRFI</th></tr></thead>
...
def main():
    ...
    tabs = st.tabs(["🔥 Today", "📋 Slate", "🧾 Full Slate", "📊 Records", "ℹ️ About"])
    ...
    with tabs[1]:
        ...
        if not slate_df.empty:
            ...
            for _, row in slate_df[slate_df["Date"].astype(str) == selected].iterrows():
                render_game_card(row, green_bet_types)

    with tabs[2]:
        today_slate = slate_df[slate_df["Date"].astype(str) == current_date].copy() if not slate_df.empty else pd.DataFrame()
        render_full_slate_summary(today_slate, green_bet_types)
