...
.summary-card{border:1px solid rgba(56,189,248,.20);border-radius:1.15rem;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.78));box-shadow:0 16px 42px rgba(0,0,0,.25);overflow:hidden;margin-bottom:1rem}.summary-head{display:flex;align-items:flex-start;justify-content:space-between;gap:.75rem;padding:.85rem .95rem;border-bottom:1px solid rgba(148,163,184,.16);background:linear-gradient(135deg,rgba(37,99,235,.18),rgba(14,165,233,.08))}.summary-title{font-size:1.05rem;font-weight:950;color:#fff;letter-spacing:-.02em}.summary-sub{font-size:.74rem;color:#94a3b8;font-weight:800;margin-top:.15rem}.summary-count{padding:.28rem .56rem;border-radius:999px;color:#dbeafe;background:rgba(37,99,235,.18);border:1px solid rgba(96,165,250,.24);font-size:.72rem;font-weight:950;white-space:nowrap}.summary-scroll{overflow-x:auto}.slate-summary-table{width:100%;border-collapse:separate;border-spacing:0;min-width:760px}.slate-summary-table th{position:sticky;top:0;background:rgba(15,23,42,.96);color:#93c5fd;text-align:left;text-transform:uppercase;letter-spacing:.045em;font-size:.66rem;font-weight:950;padding:.62rem .7rem;border-bottom:1px solid rgba(148,163,184,.18)}.slate-summary-table td{padding:.64rem .7rem;border-bottom:1px solid rgba(148,163,184,.10);color:#e5e7eb;font-size:.78rem;vertical-align:middle}.slate-summary-table tr:last-child td{border-bottom:none}.slate-summary-table tr:nth-child(even) td{background:rgba(30,41,59,.24)}.summary-game{font-weight:950;color:#fff;line-height:1.2}.summary-pill{display:inline-flex;align-items:center;border-radius:999px;padding:.22rem .48rem;background:rgba(34,197,94,.13);border:1px solid rgba(34,197,94,.26);color:#bbf7d0;font-size:.70rem;font-weight:950;white-space:nowrap}.summary-pill.blue{background:rgba(14,165,233,.13);border-color:rgba(56,189,248,.28);color:#bae6fd}.summary-pill.yellow{background:rgba(250,204,21,.12);border-color:rgba(250,204,21,.25);color:#fde68a}.summary-muted{display:block;color:#94a3b8;font-size:.68rem;margin-top:.10rem;line-height:1.2}.screenshot-note{color:#94a3b8;font-size:.73rem;line-height:1.35;margin:.2rem 0 .8rem}
...
def summarize_k_for_table(value):
    text = safe_text(value)
    if not text or text.lower().startswith("no "):
        return "—"
    details = extract_k_details(text)
    ...
    projection = details.get("projection", "")
    bet_type = display_bet_type(details.get("bet_type") or normalize_bet_type_text(text) or "")
    line_value = details.get("line", "")
    top = pitcher
    if projection:
        top += f" · {projection} K"
    ...
    if line_value:
        sub_parts.append(f"Line {line_value}")
    sub = " · ".join(sub_parts)
    return f"{esc(top)}<span class='summary-muted'>{esc(sub)}</span>" if sub else esc(top)


def render_full_slate_summary(today_slate):
    st.markdown('<div class="section-title">Full Slate Summary</div>', unsafe_allow_html=True)
    st.markdown('<div class="screenshot-note">Screenshot-friendly snapshot of every saved game: better moneyline, moneyline %, pitcher K projections, and NRFI/YRFI.</div>', unsafe_allow_html=True)
    ...
    for _, row in today_slate.iterrows():
        game = make_game_label(row)
        better_ml = safe_text(row.get("Better ML", ""), "—")
        ml_grade = display_bet_type(row.get("ML Grade", ""))
        ml_odds = safe_text(row.get("ML Odds", ""))
        ml_pct = get_better_ml_percent(row)
        nrfi = display_bet_type(row.get("NRFI Grade", "")) or "—"
        away_k = summarize_k_for_table(row.get("Away Pitcher K + Grade", ""))
        home_k = summarize_k_for_table(row.get("Home Pitcher K + Grade", ""))
        ml_sub = " · ".join([x for x in [ml_grade, ml_odds] if x])
        ml_cell = f"<span class='summary-pill'>{esc(better_ml)}</span>"
        if ml_sub:
            ml_cell += f"<span class='summary-muted'>{esc(ml_sub)}</span>"
        nrfi_cls = "summary-pill yellow" if "YRFI" in nrfi.upper() else "summary-pill blue"
        rows_html.append(f"""
        <tr>
          <td><div class="summary-game">{esc(game)}</div></td>
          <td>{ml_cell}</td>
          <td><span class="summary-pill blue">{esc(ml_pct)}</span></td>
          <td>{away_k}</td>
          <td>{home_k}</td>
          <td><span class="{nrfi_cls}">{esc(nrfi)}</span></td>
        </tr>
        """)
    ...
    <div class="summary-card">
      ...
      <div class="summary-scroll">
        <table class="slate-summary-table">
          <thead><tr><th>Game</th><th>Better ML</th><th>ML %</th><th>Away Pitcher K</th><th>Home Pitcher K</th><th>NRFI/YRFI</th></tr></thead>
...
def main():
    ...
    with tabs[2]:
        today_slate = slate_df[slate_df["Date"].astype(str) == current_date].copy() if not slate_df.empty else pd.DataFrame()
        render_full_slate_summary(today_slate)
