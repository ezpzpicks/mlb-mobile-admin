...
.logo-wrap {display:flex;justify-content:center;margin:.25rem 0 .75rem;}
...
.section-title {font-size:1.2rem;font-weight:950;letter-spacing:-.02em;margin:1.2rem 0 .65rem;color:#f8fafc}.pick-card,.game-card,.record-card,.info-card {border:1px solid var(--border);border-radius:1.15rem;padding:.92rem;margin-bottom:.72rem;background:rgba(15,23,42,.88);box-shadow:0 10px 30px rgba(0,0,0,.2)}.pick-card {position:relative;overflow:hidden}.pick-card:before{content:"";position:absolute;left:0;top:0;bottom:0;width:5px;background:var(--accent,#38bdf8)}
.card-top {display:flex;align-items:center;justify-content:space-between;gap:.5rem}.rank {min-width:2.1rem;height:2.1rem;border-radius:999px;display:inline-flex;align-items:center;justify-content:center;font-weight:950;background:rgba(56,189,248,.14);color:#7dd3fc;border:1px solid rgba(56,189,248,.28)}.pill {padding:.25rem .55rem;border-radius:999px;font-weight:900;font-size:.72rem;background:rgba(148,163,184,.12);border:1px solid rgba(148,163,184,.22);color:#e2e8f0;white-space:nowrap}.pick-title,.game-title{font-size:1.02rem;font-weight:950;line-height:1.25;color:#fff}.muted{color:var(--muted);font-size:.84rem;line-height:1.35}.tiny{color:#94a3b8;font-size:.74rem}.footer-note{color:#94a3b8;font-size:.74rem;line-height:1.35;margin-top:1.25rem;padding-bottom:2rem}
...
.pick-media{display:flex;align-items:center;gap:.65rem;margin:.55rem 0 .25rem}.headshot{width:48px;height:48px;object-fit:cover;border-radius:999px;background:rgba(30,41,59,.9);border:1px solid rgba(148,163,184,.24);flex:0 0 auto}.lines{display:grid;grid-template-columns:1fr;gap:.42rem;margin-top:.65rem}.chip{border-radius:.8rem;padding:.55rem .65rem;background:rgba(30,41,59,.78);border:1px solid rgba(148,163,184,.17);color:#e5e7eb;font-size:.82rem;line-height:1.3}.stTabs [data-baseweb="tab-list"]{gap:.35rem;overflow-x:auto}.stTabs [data-baseweb="tab"]{border-radius:999px;background:rgba(15,23,42,.88);border:1px solid rgba(148,163,184,.18);padding:.55rem .7rem}
@media(max-width:520px){.hero-title{font-size:1.72rem}.pick-card,.game-card,.record-card,.info-card{padding:.82rem}.block-container{padding-left:.55rem;padding-right:.55rem}.team-logo{width:30px;height:30px}.team-name{font-size:.76rem}}
</style>
...
def extract_k_line(k_summary):
    ...
    if "Line" not in text:
        return ""
    line = text.split("Line", 1)[1].strip().replace("/", "").split("(", 1)[0].strip()
...
def find_mlb_player_id(player_name):
    name = safe_text(player_name)
    if not name or name.upper() == "TBD":
        return ""
    try:
        response = requests.get(f"https://statsapi.mlb.com/api/v1/people/search?names={quote(name)}", timeout=4)
        response.raise_for_status()
        people = response.json().get("people", [])
        return str(people[0].get("id", "")) if people else ""
    except Exception:
        return ""
...
def build_best_plays(today_slate, green_bet_types):
    ...
    for _, row in today_slate.iterrows():
        ...
        if ml_grade in ["A Moneyline", "B Moneyline"] and normalize_bet_type_text(ml_grade) in green_bet_types:
            rows.append({"Play Type": ml_grade, "Game": game, "Play": safe_text(row.get("Better ML", "")), "Odds/Line": safe_text(row.get("ML Odds", "")), "Score": static_score(ml_grade)})
        ...
        if nrfi in ["ELITE NRFI", "STRONG NRFI", "YRFI"] and normalize_bet_type_text(nrfi) in green_bet_types:
            rows.append({"Play Type": nrfi, "Game": game, "Play": nrfi, "Odds/Line": "", "Score": static_score(nrfi)})
        for play_col, score_col in [("Away Pitcher K + Grade", "Away Pitcher K Score"), ("Home Pitcher K + Grade", "Home Pitcher K Score")]:
            ...
            if bet_type in green_bet_types and "PASS" not in upper:
                ...
                rows.append({"Play Type": bet_type.title(), "Game": game, "Play": text, "Odds/Line": extract_k_line(text), "Score": k_score})
    if not rows:
        return pd.DataFrame(columns=["Play Type", "Game", "Play", "Odds/Line", "Score"])
...
def render_hero():
    if os.path.exists(LOGO_FILE):
        ...
        st.image(LOGO_FILE, width=125)
        st.markdown('</div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="hero"><div class="hero-title">EZPZ Picks</div><div class="hero-subtitle">Algorithm-based MLB betting board focused on green/winning bet types.</div><div class="badge-row"><span class="badge">Green Bets First</span><span class="badge">Best Plays</span><span class="badge">Units + ROI</span></div></div>
    """, unsafe_allow_html=True)
...
def render_pick_card(rank, row):
    play_type = safe_text(row.get("Play Type", "Pick")); game = safe_text(row.get("Game", "")); play = safe_text(row.get("Play", "")); odds = safe_text(row.get("Odds/Line", "")); score = safe_float(row.get("Score", 0), 0)
    headshot_html = ""
    if any(x in play.upper() for x in ["OVER", "UNDER", "LEAN", "STRONG"]):
        pitcher = extract_pitcher_name_from_k_play(play); url = pitcher_headshot_url(pitcher)
        if url:
            headshot_html = f'<img class="headshot" src="{url}" alt="{esc(pitcher)} headshot">'
    odds_html = f"<span class='tiny'> · {esc(odds)}</span>" if odds else ""
    title = f'<div class="pick-media">{headshot_html}<div><div class="pick-title">{esc(play)}{odds_html}</div><div class="muted">{esc(game)}</div></div></div>' if headshot_html else f'<div class="pick-title">{esc(play)}{odds_html}</div><div class="muted">{esc(game)}</div>'
    st.markdown(f"""
    <div class="pick-card" style="--accent:{grade_accent(play_type + ' ' + play)};"><div class="card-top"><span class="rank">#{rank}</span><span class="pill">{esc(play_type)} · Score {score:.0f}</span></div>{matchup_strip(game)}{title}</div>
    """, unsafe_allow_html=True)


def render_game_card(row):
    game = make_game_label(row); better_ml = safe_text(row.get("Better ML", ""), "No ML play"); ml_grade = safe_text(row.get("ML Grade", "")); ml_odds = safe_text(row.get("ML Odds", "")); nrfi = safe_text(row.get("NRFI Grade", ""), "No NRFI/YRFI play"); away_k = safe_text(row.get("Away Pitcher K + Grade", ""), "No away K play"); home_k = safe_text(row.get("Home Pitcher K + Grade", ""), "No home K play")
    ml_line = better_ml + (f" · {ml_grade}" if ml_grade else "") + (f" · {ml_odds}" if ml_odds else "")
    st.markdown(f"""
    <div class="game-card"><div class="game-title">{esc(game)}</div>{matchup_strip(game)}<div class="lines"><div class="chip"><strong>Moneyline:</strong> {esc(ml_line)}</div><div class="chip"><strong>NRFI/YRFI:</strong> {esc(nrfi)}</div><div class="chip"><strong>Away K:</strong> {esc(away_k)}</div><div class="chip"><strong>Home K:</strong> {esc(home_k)}</div></div></div>
    """, unsafe_allow_html=True)
...
def render_records_table(summary_df):
    ...
    st.dataframe(view[["Bet Type", "Record Status", "Record", "Win %", "Units", "ROI %"]], use_container_width=True, hide_index=True)
...
def main():
    ...
    with tabs[0]:
        ...
        st.markdown(f"<div class='tiny'>Last refreshed: {datetime.now().strftime('%b %d, %Y %I:%M %p')}</div>", unsafe_allow_html=True)
