...
SLATE_COLUMNS = [
    ...
    # Optional percent columns. If any of these exist in your daily_slate tab,
    # the Full Slate Snapshot tab will use them automatically.
...
.pick-media{display:flex;align-items:center;gap:.65rem;margin:.55rem 0 .25rem}.headshot{width:48px;height:48px;object-fit:cover;object-position:center top;transform:scale(1.12);border-radius:999px;background:rgba(30,41,59,.9);border:1px solid rgba(148,163,184,.24);flex:0 0 auto}.lines{display:grid;grid-template-columns:1fr;gap:.42rem;margin-top:.65rem}.chip{border-radius:.8rem;padding:.55rem .65rem;background:rgba(30,41,59,.78);border:1px solid rgba(148,163,184,.17);color:#e5e7eb;font-size:.82rem;line-height:1.3}.chip.green-prop{background:linear-gradient(135deg,rgba(20,83,45,.55),rgba(22,101,52,.22));border-color:rgba(34,197,94,.48);box-shadow:inset 0 0 0 1px rgba(134,239,172,.08)}.k-detail-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.42rem;margin-top:.55rem}.k-detail{border-radius:.75rem;background:rgba(2,6,23,.38);border:1px solid rgba(148,163,184,.16);padding:.48rem .55rem}.k-label{display:block;color:#94a3b8;font-size:.64rem;font-weight:900;text-transform:uppercase;letter-spacing:.04em}.k-value{display:block;color:#f8fafc;font-size:.9rem;font-weight:950;margin-top:.08rem}.breakdown-grid{display:grid;grid-template-columns:1fr;gap:.55rem}.breakdown-card{border:1px solid rgba(148,163,184,.18);background:rgba(15,23,42,.86);border-radius:1rem;padding:.75rem}.breakdown-card.green{border-color:rgba(34,197,94,.42);background:linear-gradient(135deg,rgba(20,83,45,.32),rgba(15,23,42,.86))}.breakdown-card.yellow{border-color:rgba(250,204,21,.34)}.breakdown-card.red{border-color:rgba(239,68,68,.34)}.breakdown-title{font-weight:950;color:#fff;text-transform:uppercase}.breakdown-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.4rem;margin-top:.55rem}.game-card .pick-title{font-size:.78rem!important;line-height:1.18}.game-card .k-detail-grid{gap:.30rem;margin-top:.42rem}.game-card .k-detail{padding:.32rem .40rem;border-radius:.58rem}.game-card .k-label{font-size:.52rem}.game-card .k-value{font-size:.68rem;line-height:1.12}.game-card .chip{font-size:.76rem;padding:.46rem .55rem}/* Premium centered navigation tabs */
.stTabs [data-baseweb="tab-list"]{justify-content:center;gap:.55rem;overflow-x:auto;padding:.42rem;margin:.4rem auto .8rem;border:1px solid rgba(56,189,248,.18);border-radius:999px;background:linear-gradient(135deg,rgba(15,23,42,.80),rgba(2,6,23,.52));box-shadow:0 12px 30px rgba(0,0,0,.18);scrollbar-width:none}.stTabs [data-baseweb="tab-list"]::-webkit-scrollbar{display:none}.stTabs [data-baseweb="tab"]{position:relative;height:42px;border-radius:999px;background:rgba(15,23,42,.72);border:1px solid rgba(148,163,184,.18);padding:.52rem .86rem;color:#cbd5e1;font-weight:900;transition:transform .18s ease,background .18s ease,border-color .18s ease,box-shadow .18s ease}.stTabs [data-baseweb="tab"] p{font-weight:950;font-size:.88rem;letter-spacing:-.01em}.stTabs [data-baseweb="tab"]:hover{transform:translateY(-1px);background:rgba(56,189,248,.14);border-color:rgba(56,189,248,.42);box-shadow:0 8px 20px rgba(14,165,233,.14)}.stTabs [aria-selected="true"]{background:linear-gradient(135deg,#2563eb,#38bdf8)!important;color:#fff!important;border-color:transparent!important;box-shadow:0 10px 28px rgba(37,99,235,.42),inset 0 1px 0 rgba(255,255,255,.22)!important;transform:translateY(-1px) scale(1.015)}.stTabs [aria-selected="true"]:after{content:"";position:absolute;inset:-2px;z-index:-1;border-radius:999px;background:linear-gradient(135deg,rgba(56,189,248,.42),rgba(37,99,235,.08));filter:blur(9px);opacity:.75;animation:ezpzTabGlow 2.8s ease-in-out infinite}.stTabs [data-baseweb="tab-highlight"],.stTabs [data-baseweb="tab-border"]{display:none}@keyframes ezpzTabGlow{0%,100%{opacity:.45;transform:scale(.98)}50%{opacity:.85;transform:scale(1.04)}}
/* Full slate snapshot polish */
.snapshot-note{color:#94a3b8;font-size:.76rem;line-height:1.35;margin:-.15rem 0 .65rem}.snapshot-wrap{border:1px solid rgba(56,189,248,.20);border-radius:1.05rem;background:linear-gradient(135deg,rgba(15,23,42,.96),rgba(2,6,23,.78));box-shadow:0 14px 36px rgba(0,0,0,.22);padding:.75rem;margin-bottom:1rem}.snapshot-title-row{display:flex;justify-content:space-between;align-items:center;gap:.7rem;margin-bottom:.55rem}.snapshot-title{font-size:.98rem;font-weight:950;color:#fff}.snapshot-count{padding:.22rem .5rem;border-radius:999px;color:#dbeafe;background:rgba(37,99,235,.14);border:1px solid rgba(96,165,250,.20);font-size:.68rem;font-weight:950;white-space:nowrap}.green-key{display:inline-block;border-radius:999px;padding:.16rem .42rem;background:rgba(34,197,94,.14);border:1px solid rgba(34,197,94,.34);color:#bbf7d0;font-size:.68rem;font-weight:950}
...
def first_existing_value(row, possible_columns):
...
def get_better_ml_percent(row):
    ...
    if direct:
        return format_percent_text(direct)
    ...
    if better_team and away and better_team.lower() == away.lower() and away_pct:
        return format_percent_text(away_pct)
    if better_team and home and better_team.lower() == home.lower() and home_pct:
        return format_percent_text(home_pct)
    return "—"
...
def compact_k_summary(value, green_bet_types=None):
    green_bet_types = green_bet_types or set()
    text = safe_text(value)
    if not text or text.lower().startswith("no ") or "PASS" in text.upper():
        return "—"
    details = extract_k_details(text)
    pitcher = short_person_name(details.get("pitcher", ""))
    ...
    extra = []
    if bet_type:
        extra.append(bet_type.replace("STRONG ", "S ").replace("LEAN ", "L "))
    if line_value:
        extra.append(f"L{line_value}")
    if extra:
        main += " (" + " · ".join(extra) + ")"
    return ("🟢 " + main) if bet_key and bet_key in green_bet_types else main


def compact_ml_summary(row, green_bet_types=None):
    green_bet_types = green_bet_types or set()
    better_ml = safe_text(row.get("Better ML", ""), "—")
    ml_grade_raw = safe_text(row.get("ML Grade", ""))
    ml_odds = safe_text(row.get("ML Odds", ""))
    text = better_ml + (f" {ml_odds}" if ml_odds else "")
    return ("🟢 " + text) if is_green_play(ml_grade_raw, green_bet_types) else text


def compact_nrfi_summary(value, green_bet_types=None):
    text = display_bet_type(value) or "—"
    return ("🟢 " + text) if is_green_play(value, green_bet_types or set()) else text
...
def render_full_slate_summary(today_slate, green_bet_types=None):
    ...
    st.markdown('<div class="snapshot-note">Screenshot-friendly overview. The only green markers shown are for bet types currently winning in your tracked record.</div>', unsafe_allow_html=True)
    ...
    for _, row in today_slate.iterrows():
        snapshot_rows.append({
            "Game": make_game_label(row),
            "Better ML": compact_ml_summary(row, green_bet_types),
            "ML %": get_better_ml_percent(row),
            "Away K": compact_k_summary(row.get("Away Pitcher K + Grade", ""), green_bet_types),
            "Home K": compact_k_summary(row.get("Home Pitcher K + Grade", ""), green_bet_types),
            "NRFI/YRFI": compact_nrfi_summary(row.get("NRFI Grade", ""), green_bet_types),
        })
    ...
    <div class="snapshot-wrap">
      <div class="snapshot-title-row">
        <div class="snapshot-title">EZPZ Full Slate Snapshot</div>
        <div class="snapshot-count">{len(snapshot_df)} Games</div>
      </div>
      <span class="green-key">🟢 Green bet type</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.dataframe(
        ...
        height=min(720, 38 + (len(snapshot_df) + 1) * 36),
        column_config={
            "Game": st.column_config.TextColumn("Game", width="medium"),
            "Better ML": st.column_config.TextColumn("Better ML", width="small"),
            "ML %": st.column_config.TextColumn("ML %", width="small"),
            "Away K": st.column_config.TextColumn("Away K", width="medium"),
            "Home K": st.column_config.TextColumn("Home K", width="medium"),
            "NRFI/YRFI": st.column_config.TextColumn("NRFI/YRFI", width="small"),
        },
    )
