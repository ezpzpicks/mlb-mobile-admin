...
.logo-wrap {display:flex;justify-content:center;margin:.25rem 0 .75rem;} .logo-wrap img{border-radius:1.1rem;box-shadow:0 12px 32px rgba(0,0,0,.35);}
...
def render_pick_card(rank, row):
    ...
    if is_k_play:
        ...
        line_value = details.get("line", "") or odds.replace("Line", "").strip()
...
def render_game_card(row, green_bet_types=None):
    ...
    chips = "".join([
        ...
        prop_chip("Away K", away_k, is_green_play(away_k, green_bet_types)),
        prop_chip("Home K", home_k, is_green_play(home_k, green_bet_types)),
    ])
    st.markdown(f"""
