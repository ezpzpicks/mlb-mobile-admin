...
def _install_fast_save(builder: Any) -> None:
    """Commit one explicit CFB save as a single Turso transaction.

    A saved matchup updates several logical datasets: slate, tracker, both team
    personnel snapshots, and the Covers starter/injury history. Committing each
    dataset independently makes the button wait on several network round trips.
...
def install_interactive_recovery(builder: Any) -> None:
    """Install selected-matchup responsiveness, Covers recovery, and fast saves."""
