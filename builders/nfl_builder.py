...
HOME_FIELD_VENUE_REGRESSION_GAMES = 40.0
...
def _rolling_home_field_model(season: int, projection_week: int) -> dict[str, Any]:
    ...
    strength. Team/venue residuals are then shrunk toward zero according to the
    effective number of weighted home games instead of being forced into a tight
    fixed cap. A broad emergency limit remains only to contain corrupted or
    pathological data.
    """
    season = int(season)
    ...
    try:
        ...
        beta = np.linalg.lstsq(
            ...
            weighted_design.T @ weighted_outcome,
            rcond=None,
        )[0]
    ...
    team_adjustment_details: dict[str, dict[str, float]] = {}
    ...
    for team in teams:
        ...
        team_residual = venue_residual[mask]
        weight_sum = float(team_weight.sum())
        weight_square_sum = float(np.square(team_weight).sum())
        if weight_sum <= 0 or weight_square_sum <= 0:
            continue

        raw_adjustment = float(np.average(team_residual, weights=team_weight))
        effective_home_games = float((weight_sum ** 2) / weight_square_sum)
        reliability_weight = float(
            effective_home_games / (effective_home_games + HOME_FIELD_VENUE_REGRESSION_GAMES)
        )
        applied_adjustment = raw_adjustment * reliability_weight
        applied_adjustment = float(clamp(
            applied_adjustment,
            -HOME_FIELD_VENUE_EMERGENCY_LIMIT,
            HOME_FIELD_VENUE_EMERGENCY_LIMIT,
        ))

        team_adjustments[team] = round(applied_adjustment, 3)
        team_adjustment_details[team] = {
            ...
            "effective_home_games": round(effective_home_games, 1),
            "weighted_home_games": round(weight_sum, 1),
            "actual_home_games": int(mask.sum()),
        }
    ...
    return {
        ...
        "team_adjustment_details": team_adjustment_details,
        "games": n_games,
...
def _automatic_home_field_advantage(
    ...
    if _is_neutral_schedule_row(schedule_row):
        return {
            ...
            "raw_venue_adjustment": 0.0,
            "venue_reliability": 1.0,
            "venue_effective_games": 0.0,
            "games": 0,
            "confidence": 100.0,
            "source": "Neutral-site game",
        }
    ...
    venue_adjustment = _num(model.get("team_adjustments", {}).get(normalized_home, 0.0), 0.0)
    details = model.get("team_adjustment_details", {}).get(normalized_home, {})
    raw_venue_adjustment = _num(details.get("raw_adjustment"), venue_adjustment)
    venue_reliability = _num(details.get("reliability_weight"), 0.0)
    venue_effective_games = _num(details.get("effective_home_games"), 0.0)
    actual_home_games = _int(details.get("actual_home_games"), 0)

    raw_total = league_points + venue_adjustment
    value = float(clamp(raw_total, HOME_FIELD_TOTAL_MIN, HOME_FIELD_TOTAL_MAX))
    source = (
        ...
        f"(raw {raw_venue_adjustment:+.2f}, {venue_reliability:.0%} reliability, "
        f"{venue_effective_games:.1f} effective/{actual_home_games} actual home games)"
    )
    return {
        "value": round(value, 2),
        "league_points": round(league_points, 2),
        "venue_adjustment": round(venue_adjustment, 2),
        "raw_venue_adjustment": round(raw_venue_adjustment, 2),
        "venue_reliability": round(venue_reliability, 3),
        "venue_effective_games": round(venue_effective_games, 1),
...
def _evaluate_prop_rows(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in rows.iterrows():
        ...
        projection_edge_value = abs(projection - line)
        grade = _grade_prop(
        ...
        item.update({
            ...
            "Probability Edge": round(probability_edge_value, 4), "Projection Edge": round(projection_edge_value, 2),
...
def _game_teams_from_label(value: Any) -> tuple[str, str]:
    """Recover away/home abbreviations from the tracker game label."""
    game = _safe_text(value)
    if not game:
        return "", ""
    parts = re.split(r"\s+(?:at|@|vs\.?)\s+", game, maxsplit=1, flags=re.IGNORECASE)
    if len(parts) != 2:
        return "", ""
    return _normalize_team(parts[0]), _normalize_team(parts[1])


def _grade_completed_game_bet(row: pd.Series | dict[str, Any], game_row: pd.Series | dict[str, Any]) -> str:
    """Grade one saved NFL game wager from the final score."""
    away_team = _normalize_team(game_row.get("Away Team", ""))
    home_team = _normalize_team(game_row.get("Home Team", ""))
    away_score = _num(game_row.get("Away Score", np.nan), np.nan)
    home_score = _num(game_row.get("Home Score", np.nan), np.nan)
    if not (math.isfinite(away_score) and math.isfinite(home_score)):
        return ""

    bet_type = _safe_text(row.get("Bet Type", "")).strip().lower()
    selection = _safe_text(row.get("Selection", "")).strip()

    if bet_type == "total":
        match = re.search(r"\b(over|under)\s+([+-]?\d+(?:\.\d+)?)", selection, flags=re.IGNORECASE)
        if not match:
            return ""
    ...
    if bet_type == "spread":
        match = re.match(r"^(.+?)\s+([+-]\d+(?:\.\d+)?)\s*$", selection)
        if not match:
            return ""
        selected_team = _normalize_team(match.group(1))
        spread = float(match.group(2))
        if selected_team == away_team:
            adjusted_margin = away_score + spread - home_score
        elif selected_team == home_team:
            adjusted_margin = home_score + spread - away_score
        else:
            return ""
    ...
    if bet_type == "moneyline":
        cleaned = re.sub(r"\s+(?:moneyline|ml)\s*$", "", selection, flags=re.IGNORECASE).strip()
        selected_team = _normalize_team(cleaned)
        if selected_team not in [away_team, home_team]:
            first_token = _normalize_team(selection.split()[0]) if selection.split() else ""
            selected_team = first_token if first_token in [away_team, home_team] else ""
        if not selected_team:
            return ""
        if abs(away_score - home_score) < 1e-9:
            return "Push"
        winner = away_team if away_score > home_score else home_team
        return "Win" if selected_team == winner else "Loss"

    return ""


def _auto_update_game_tracker() -> tuple[int, str]:
    """Refresh final NFL scores and grade any unfinished game wagers."""
    if not sheets_ready():
        return 0, "Google Sheets is not configured."
    tracker = read_sheet(TRACKER_TAB, TRACKER_COLUMNS)
    if tracker is None or tracker.empty:
        return 0, "No game tracker rows yet."
    if "Result" in tracker.columns:
        tracker["Result"] = tracker["Result"].astype(object)

    schedule_cache: dict[int, pd.DataFrame] = {}
    updated_rows = 0

    for index, row in tracker.iterrows():
        current_result = _safe_text(row.get("Result", "")).strip().lower()
        if current_result not in ["", "pending"]:
            continue
        season = _int(row.get("Season", 0), 0)
        week = _int(row.get("Week", 0), 0)
        if season <= 0:
        ...
        if season not in schedule_cache:
            try:
                schedule_cache[season] = _schedule_for_season(season, refresh=True)
            except Exception:
                schedule_cache[season] = pd.DataFrame(columns=SCHEDULE_COLUMNS)
        schedule = schedule_cache[season]
        if schedule is None or schedule.empty:
            continue

        game_id = _safe_text(row.get("Game ID", ""))
        matches = pd.DataFrame()
        if game_id and "Game ID" in schedule.columns:
            matches = schedule[schedule["Game ID"].astype(str) == str(game_id)].copy()

        if matches.empty:
            away_team, home_team = _game_teams_from_label(row.get("Game", ""))
            fallback = schedule.copy()
            if week > 0 and "Week" in fallback.columns:
                fallback = fallback[pd.to_numeric(fallback["Week"], errors="coerce") == week]
            if away_team and home_team:
                fallback = fallback[
                    (fallback["Away Team"].astype(str).map(_normalize_team) == away_team)
                    & (fallback["Home Team"].astype(str).map(_normalize_team) == home_team)
                ]
            matches = fallback
        ...
        result = _grade_completed_game_bet(row, game_row)
        if result in ["Win", "Loss", "Push"]:
            tracker.at[index, "Result"] = result
            updated_rows += 1

    if updated_rows:
        write_sheet(TRACKER_TAB, tracker, TRACKER_COLUMNS)
    return updated_rows, f"Updated {updated_rows} completed game bet result(s)."


def _render_game_tracker() -> None:
    st.subheader("NFL Bet Tracker")
    try:
        updated, message = _auto_update_game_tracker()
        if updated:
            st.success(message)
    except Exception as exc:
        st.warning(f"Automatic game-result update could not finish: {exc}")
    dataframe = read_sheet(TRACKER_TAB, TRACKER_COLUMNS)
    if dataframe is not None and not dataframe.empty:
        st.dataframe(dataframe.iloc[::-1], use_container_width=True, hide_index=True)
    else:
        st.info("No tracked NFL game bets yet.")
...
def _render_build() -> None:
    ...
    with st.expander("Advanced matchup adjustments", expanded=False):
        override_home_field = st.checkbox(
            ...
            help="Leave off to use the rolling league baseline plus the confidence-weighted venue estimate. Turn on only for a known site-specific exception.",
        )
        if override_home_field:
            home_field = st.number_input(
                "Manual home-field points", value=float(automatic_home_field), step=0.1,
                min_value=-1.0, max_value=5.0, key=f"nfl_hfa_{market_key}",
            )
        else:
