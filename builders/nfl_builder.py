...
MODEL_VERSION = "nfl-v1.1-render-stability-2026-07-15"
DEFAULT_SEASON = 2026
...
PLAYER_STATS_REQUIRED_COLUMNS = [
    "season_type", "week", "player_id", "gsis_id", "player_display_name",
    "player_name", "full_name", "recent_team", "team", "position", "position_group",
    "attempts", "passing_epa", "passing_yards", "passing_tds", "interceptions",
    "carries", "rushing_epa", "rushing_yards", "rushing_tds", "targets",
    "receptions", "receiving_epa", "receiving_yards", "receiving_tds",
]
...
def _load_pbp_season(season: int) -> pd.DataFrame:
    ...
    try:
        try:
            frame = nfl.load_pbp(int(season), columns=PBP_REQUIRED_COLUMNS)
        except TypeError:
            frame = nfl.load_pbp(int(season))
        return _compact_to_pandas(frame, PBP_REQUIRED_COLUMNS)
    except Exception:
        return pd.DataFrame()
...
def _render_build() -> None:
    ...
    lineup_source_key = f"nfl_live_lineup_source_{season}_{week}_{away_team}_{home_team}"
    live_lineups_ready = lineup_source_key in st.session_state

    if ratings is None or ratings.empty or str(away_rating.get("Source", "")).lower().startswith("neutral"):
        st.info(
            "Initialize the model in Setup first. Until then, the builder uses editable generic lineup slots "
            "and does not start the heavier live-player downloads."
        )

    load_label = "Refresh expected lineups and injuries" if live_lineups_ready else "Load expected lineups and injuries"
    if st.button(load_label, key=f"load_{lineup_source_key}", use_container_width=True):
        try:
            with st.spinner("Loading compact nflverse depth charts, injuries, and player values..."):
                depth_live = _latest_depth_chart(int(season))
                injury_live = _injury_lookup(int(season), int(week))
                values_live = _blend_player_values(int(season), int(week))
                away_seed = _auto_lineup(away_team, int(season), int(week), depth_live, injury_live, values_live)
                home_seed = _auto_lineup(home_team, int(season), int(week), depth_live, injury_live, values_live)
            st.session_state[lineup_source_key] = {"away": away_seed, "home": home_seed}
            st.success("Expected lineups loaded. They will remain available during this app session.")
            st.rerun()
        except Exception as exc:
            st.warning(f"Live lineup data could not be loaded. Generic editable slots remain available: {exc}")

    saved_lineups = st.session_state.get(lineup_source_key, {})
    if saved_lineups:
        depth = pd.DataFrame()
        injury_lookup = {}
        player_values = {}
        away_seed_lineup = saved_lineups.get("away", pd.DataFrame())
        home_seed_lineup = saved_lineups.get("home", pd.DataFrame())
        st.caption("Live lineup source loaded for this matchup. Use Refresh above for newer data.")
    else:
        depth = pd.DataFrame()
        injury_lookup = {}
        player_values = {}
        away_seed_lineup = pd.DataFrame()
        home_seed_lineup = pd.DataFrame()

    away_tab, home_tab = st.tabs([f"{away_team} lineup", f"{home_team} lineup"])
    with away_tab:
        away_lineup, away_lineup_summary = _lineup_editor(
            away_team, int(season), int(week), market_key, depth, injury_lookup, player_values, away_seed_lineup
        )
    with home_tab:
        home_lineup, home_lineup_summary = _lineup_editor(
            home_team, int(season), int(week), market_key, depth, injury_lookup, player_values, home_seed_lineup
        )
...
def _render_setup() -> None:
    ...
    st.caption("NFL data now uses a filesystem cache and compact column selection to reduce Render memory pressure.")
    ...
    if st.button("Clear NFL download cache", use_container_width=True):
        try:
            ...
            for key in list(st.session_state):
                if str(key).startswith("nfl_live_lineup_source_"):
                    del st.session_state[key]
            st.success("NFL download and lineup-session caches cleared.")
        except Exception as exc:
