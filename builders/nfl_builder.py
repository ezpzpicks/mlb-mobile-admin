...
MODEL_VERSION = "nfl-v2.0-auto-slate-props-2026-07-16"
DEFAULT_SEASON = 2026
...
def _project_player_markets(
    ...
    def add_market(market: str, projection: float, matchup_index: float, attempts: float = 0.0, targets: float = 0.0, receptions: float = 0.0, routes: float = 0.0, efficiency: float = 0.0, reason: str = "") -> None:
        role_conf = _role_confidence({**profile, "position": pos}, play_probability, slot, market)
        ...
        rows.append({
            ...
            "Confluence": reason, "_sd": _prop_sd(market, projection, reliability),
        })

    if pos == "QB":
        attempt_share = _num(profile.get("attempt_share", np.nan), np.nan)
        ...
        qb_carry_share = _num(profile.get("carry_share", defaults["carry_share"]), defaults["carry_share"])
        ...
        rb_snap = _num(profile.get("snap_share", defaults.get("snap_share", 0.55)), defaults.get("snap_share", 0.55))
...
def _build_game_prop_rows(
    ...
    rows: list[dict[str, Any]] = []
    for team, opponent, home_away, lineup, rating, opponent_rating in [
        (away_team, home_team, "Away", away_lineup, away_rating, home_rating),
        (home_team, away_team, "Home", home_lineup, home_rating, away_rating),
    ]:
        skill = lineup[
        ...
        for _, player_row in skill.iterrows():
            rows.extend(_project_player_markets(
                ...
                defense_profiles, rating, opponent_rating, projection, weather_adjustment, market_lines,
            ))
    return pd.DataFrame(rows)
...
def _render_build() -> None:
    ...
    st.markdown("### Expected lineup card and injuries")
    st.caption("QB, RB1/RB2, WR1–WR3 and TE feed the prop model automatically. Every row remains editable for late inactives or role changes.")
    ...
    with st.expander("Game matchup explanation", expanded=False):
    ...
    if prop_base.empty:
        st.info("No skill-position players were resolved from the lineup card. Edit the QB/RB/WR/TE lineup rows above and the props will appear.")
    else:
        input_columns = [
            "Team", "Player", "Position", "Slot", "Market", "Projection", "Fair Line", "Market Line",
            "Over Odds", "Under Odds", "Line Source", "Projected Routes", "Projected Player Attempts",
            "Projected Targets", "Role Confidence", "Matchup Index", "Confluence",
        ]
        prop_inputs = st.data_editor(
            ...
            column_config={
                ...
                "Fair Line": st.column_config.NumberColumn(format="%.1f"),
                "Market Line": st.column_config.NumberColumn("Sportsbook Line", min_value=0.0, step=0.5, format="%.1f"),
                "Over Odds": st.column_config.NumberColumn(step=5), "Under Odds": st.column_config.NumberColumn(step=5),
                "Role Confidence": st.column_config.ProgressColumn(min_value=0, max_value=100, format="%.0f"),
                "Matchup Index": st.column_config.NumberColumn(format="%.2f"),
            },
        )
        evaluated_props = _evaluate_prop_rows(prop_inputs)
        selection_columns = [
            "Track", "Team", "Player", "Market", "Projection", "Market Line", "Pick", "Pick Odds",
            "Model Probability", "Probability Edge", "Grade", "Reliability", "Role Confidence", "Matchup Index", "Confluence",
        ]
        selected_props = st.data_editor(
            evaluated_props,
            use_container_width=True,
            hide_index=True,
            key=f"nfl_prop_selection_{market_key}",
            column_order=selection_columns,
            disabled=[column for column in selection_columns if column != "Track"],
            column_config={
                "Track": st.column_config.CheckboxColumn("Send", default=False),
                "Projection": st.column_config.NumberColumn(format="%.1f"),
                "Market Line": st.column_config.NumberColumn(format="%.1f"),
                "Model Probability": st.column_config.NumberColumn(format="%.1%%"),
                "Probability Edge": st.column_config.NumberColumn(format="%.1%%"),
                "Reliability": st.column_config.ProgressColumn(min_value=0, max_value=100, format="%.0f"),
                "Role Confidence": st.column_config.ProgressColumn(min_value=0, max_value=100, format="%.0f"),
            },
        )

    save_game_col, save_props_col = st.columns(2)
    with save_game_col:
        if st.button("Save Game Projection", type="primary", use_container_width=True, key=f"nfl_save_game_{market_key}"):
            if append_row(SLATE_TAB, slate_row, SLATE_COLUMNS):
                ...
                st.success("NFL game projection and lineup snapshot saved.")
    with save_props_col:
        if st.button("Save All Prop Projections", use_container_width=True, key=f"nfl_save_props_{market_key}"):
            stored = _prop_rows_for_storage(evaluated_props, slate_date_str, season, week, game_id, game, notes)
            if stored and _replace_game_rows(PROP_SLATE_TAB, PROP_PROJECTION_COLUMNS, stored, game_id, slate_date_str):
                st.success(f"Saved {len(stored)} player prop projections.")
            elif not stored:
                st.info("There are no prop projections to save yet.")

    st.markdown("### Send plays to trackers")
    spread_selected = st.checkbox(f"{spread_grade} — {spread_pick}", value=spread_grade in ["A Spread", "B Spread"], key=f"nfl_spread_pick_{market_key}")
    total_selected = st.checkbox(f"{total_grade} — {total_pick}", value=total_grade in ["Strong Over", "Strong Under", "Over", "Under"], key=f"nfl_total_pick_{market_key}")
    ml_selected = st.checkbox(f"{ml_grade} — {ml_pick}", value=ml_grade in ["A Moneyline", "B Moneyline"], key=f"nfl_ml_pick_{market_key}")

    game_tracker_col, prop_tracker_col = st.columns(2)
    with game_tracker_col:
        if st.button("Send Selected Game Plays", use_container_width=True, key=f"nfl_send_game_{market_key}"):
            plays = []
            if spread_selected:
            ...
            st.success(f"Sent {sent} NFL game play(s) to the tracker.") if sent else st.info("No game plays were selected.")

    with prop_tracker_col:
        if st.button("Send Selected Props", use_container_width=True, key=f"nfl_send_props_{market_key}"):
            chosen_indexes = selected_props.index[selected_props["Track"].fillna(False).astype(bool)].tolist() if selected_props is not None and not selected_props.empty and "Track" in selected_props.columns else []
            chosen = evaluated_props.loc[chosen_indexes].copy() if chosen_indexes else pd.DataFrame()
            sent = 0
            for item in _prop_rows_for_storage(chosen, slate_date_str, season, week, game_id, game, notes):
                item.update({
                ...
                sent += int(bool(append_row(PROP_TRACKER_TAB, item, PROP_TRACKER_COLUMNS)))
            st.success(f"Sent {sent} NFL prop play(s) to the tracker.") if sent else st.info("No props were selected or no sportsbook lines are available.")
...
def render() -> None:
    st.caption("NFL v2 automated slate • game markets • lineup-driven QB/RB/WR/TE props")
    page = st.radio(
        "NFL section",
