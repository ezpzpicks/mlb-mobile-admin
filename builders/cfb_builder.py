...
def evaluate_game(game: pd.Series, ratings: pd.DataFrame, away_personnel: Personnel, home_personnel: Personnel,
    ...
    # ESPN/open-data schedule frames can use pandas StringDtype for an entire row.
    # Assigning a float such as -6.5 into that Series raises:
    #   TypeError: Invalid value '-6.5' for dtype 'str'
    # Rebuild the matchup row as an object-backed Series before applying the
    # user's numeric market overrides. This also keeps booleans, dates, and
    # missing values in their native types instead of coercing the whole row
    # to text.
    if isinstance(game, pd.Series):
        game_values = game.to_dict()
    else:
        game_values = dict(game)

    market_home_spread = float(_num(market_home_spread, 0.0))
    market_total = float(_num(market_total, 56.0))
    away_ml = float(_num(away_ml, 0.0))
    home_ml = float(_num(home_ml, 0.0))
    game_values.update({
    ...
    game = pd.Series(game_values, dtype=object)
