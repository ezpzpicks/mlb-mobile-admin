...
MODEL_VERSION = "cfb-v1.2-free-no-key-score-distribution-2026-07-18"
DEFAULT_SEASON = 2026
...
def _espn_events(season: int) -> list[dict[str, Any]]:
    ...
    for season_type in (2, 3):
        payload = _public_json_get(
            ...
            optional=True,
            max_age=21600 if season >= date.today().year else 86400 * 14,
        )
        for event in (payload.get("events", []) if isinstance(payload, dict) else []):
            events[_text(event.get("id"))] = event
    # Some ESPN responses return only the active week. Week-by-week is the fallback.
    if len(events) < 100:
...
def _download_open_asset(tag: str, season: int, preferred_tokens: tuple[str, ...]) -> Path | None:
    ...
    try:
        with requests.get(
            ...
            stream=True,
            timeout=180,
            headers={"User-Agent": "EZPZ-Picks-NCAAF/1.2 public-data downloader"},
        ) as response:
            response.raise_for_status()
...
def _write(tab: str, frame: pd.DataFrame, columns: list[str]) -> None:
    ...
    for col in columns:
        if col not in output.columns:
            output[col] = ""
    write_sheet(tab, output[columns].fillna(""), columns)
...
def _flatten_season_stats(payload: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    if frame.empty: return frame
    games = pd.to_numeric(frame.get("Games", 0), errors="coerce").replace(0, np.nan)
    if "Points" in frame and "Points Per Game" not in frame: frame["Points Per Game"] = pd.to_numeric(frame["Points"], errors="coerce") / games
    if "Total Yards" in frame and "Plays" in frame: frame["Yards Per Play"] = pd.to_numeric(frame["Total Yards"], errors="coerce") / pd.to_numeric(frame["Plays"], errors="coerce").replace(0, np.nan)
    if "Third Down Conversions" in frame and "Third Down Attempts" in frame: frame["Third Down Rate"] = pd.to_numeric(frame["Third Down Conversions"], errors="coerce") / pd.to_numeric(frame["Third Down Attempts"], errors="coerce").replace(0, np.nan)
    if "Red Zone Touchdowns" in frame and "Red Zone Attempts" in frame: frame["Red Zone TD Rate"] = pd.to_numeric(frame["Red Zone Touchdowns"], errors="coerce") / pd.to_numeric(frame["Red Zone Attempts"], errors="coerce").replace(0, np.nan)
    if "Turnovers" in frame and "Plays" in frame: frame["Turnover Rate"] = pd.to_numeric(frame["Turnovers"], errors="coerce") / pd.to_numeric(frame["Plays"], errors="coerce").replace(0, np.nan)
    if "Sacks Allowed" in frame and "Plays" in frame: frame["Sack Rate Allowed"] = pd.to_numeric(frame["Sacks Allowed"], errors="coerce") / pd.to_numeric(frame["Plays"], errors="coerce").replace(0, np.nan)
    if "Sacks Created" in frame and "Plays" in frame: frame["Sack Rate Created"] = pd.to_numeric(frame["Sacks Created"], errors="coerce") / pd.to_numeric(frame["Plays"], errors="coerce").replace(0, np.nan)
    return frame
...
def _season_features(season: int, through_week: int | None = None) -> tuple[pd.DataFrame, dict[str, bool]]:
    ...
    games = pd.to_numeric(frame.get("Games", 0), errors="coerce").replace(0, np.nan)
    drives = pd.to_numeric(frame.get("Advanced Drives", np.nan), errors="coerce")
    plays = pd.to_numeric(frame.get("Advanced Plays", np.nan), errors="coerce")
    ...
    if "Points Per Drive" not in frame:
        frame["Points Per Drive"] = pd.to_numeric(frame.get("Points Per Game", np.nan), errors="coerce") / pd.to_numeric(frame.get("Possessions Per Game", np.nan), errors="coerce").replace(0, np.nan)
    if "Points Allowed Per Drive" not in frame:
        frame["Points Allowed Per Drive"] = pd.to_numeric(frame.get("Points Allowed Per Game", np.nan), errors="coerce") / pd.to_numeric(frame.get("Possessions Per Game", np.nan), errors="coerce").replace(0, np.nan)
    if "Pace Seconds Per Play" not in frame:
        frame["Pace Seconds Per Play"] = 27.5
    if "QB Continuity" not in frame:
        frame["QB Continuity"] = pd.to_numeric(frame.get("Returning Passing", np.nan), errors="coerce")
    availability = {
...
def _current_components(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    out["Current Power"] = 0.47 * off + 0.47 * defense + 0.06 * special
    return out
...
def build_team_ratings(season: int, week: int) -> pd.DataFrame:
    ...
    for team in teams:
        ...
        row = {
            ...
            "Source": "Free ESPN + SportsDataverse opponent-adjusted play-by-play, roster proxies, and results",
            "Updated": _now(),
        }
...
@st.cache_data(ttl=86400, show_spinner=False)
def _historical_hfa(season: int, home_team: str, venue_id: str, lookback: int = 5) -> tuple[float, float, int, int]:
    residuals: list[float] = []; venue_residuals: list[float] = []
    team_residuals: list[float] = []
    for year in range(season - lookback, season):
        games = _parse_games(_free_data_get("games", {"year": year}, optional=True, max_age=86400 * 30), year)
        ...
        for _, game in done.iterrows():
            ...
            if _text(game["Home Team"]) == home_team: team_residuals.append(residual)
            if venue_id and _text(game["Venue ID"]) == _text(venue_id): venue_residuals.append(residual)
    league_raw = float(np.mean(residuals)) if residuals else 1.5
    league = (48.0 * 1.5 + len(residuals) * league_raw) / (48.0 + len(residuals))
    target = venue_residuals if venue_residuals else team_residuals
    raw = float(np.mean(target)) if target else league
    venue = (40.0 * league + len(target) * raw) / (40.0 + len(target))
    return round(clamp(league, 0.25, 3.0), 3), round(clamp(venue - league, -2.5, 3.5), 3), len(residuals), len(target)
...
def _nws_weather(latitude: float, longitude: float, game_time: str) -> dict[str, float | str]:
    ...
    try:
        ...
        point = requests.get(f"https://api.weather.gov/points/{latitude:.4f},{longitude:.4f}", headers=headers, timeout=20).json()
        hourly_url = _dig(point, "properties.forecastHourly", default="")
        periods = _dig(requests.get(hourly_url, headers=headers, timeout=20).json(), "properties.periods", default=[]) or [] if hourly_url else []
        try:
        ...
        pass
    # No restricted or paid fallback is used. Missing official weather remains
    # unconfirmed so reliability falls and the user can enter a manual override.
    return {}
...
def evaluate_game(game: pd.Series, ratings: pd.DataFrame, away_personnel: Personnel, home_personnel: Personnel,
                  ...
                  away_ml: float, home_ml: float, market_availability: dict[str, bool] | None = None) -> dict[str, Any]:
    ...
    simulation = simulate_game(projection, _text(game.get("Game ID"), f"{game['Away Team']}-{game['Home Team']}"))
...
def default_personnel(team: str, rating: dict[str, Any], season: int, week: int, game_id: str) -> Personnel:
    ...
    *,
    ...
    for _, game in games.iterrows():
        try:
            ...
            away_p = default_personnel(game["Away Team"], away_rating, season, week, game["Game ID"])
            home_p = default_personnel(game["Home Team"], home_rating, season, week, game["Game ID"])
...
def _render_build() -> None:
    ...
    st.caption("Opening this page automatically loads the schedule, sportsbook markets, weather, progressive team ratings, quarterback candidates, and the full weekly projection slate.")
    ...
    with st.spinner("Refreshing the full weekly NCAAF projection slate..."):
        _, weekly_errors = _ensure_automatic_week_slate(season, week, provider, schedule, ratings)
    _settle_automatically_once(season)
    if weekly_errors:
        with st.expander(f"{len(weekly_errors)} matchup(s) used partial fallbacks", expanded=False):
            st.code("\n".join(weekly_errors))
...
def _render_slate() -> None:
    st.subheader("Automatic Weekly Slate")
    season = _current_cfb_season()
    provider = str(st.session_state.get("cfb_provider_value", ""))
    schedule = _ensure_automatic_schedule(season, provider)
    if not schedule.empty:
        week = _default_projection_week(schedule)
        ratings = _ensure_automatic_ratings(season, week)
        if not ratings.empty:
            with st.spinner("Refreshing weekly NCAAF projections..."):
                _ensure_automatic_week_slate(season, week, provider, schedule, ratings)
    frame = _sheet(SLATE_TAB, SLATE_COLUMNS)
    if frame.empty:
        st.info("No college-football projections are available yet. Open Build to refresh the free public feeds.")
        return
    season_values = sorted(pd.to_numeric(frame["Season"], errors="coerce").dropna().astype(int).unique(), reverse=True)
    selected_season = st.selectbox("Season", season_values, index=0, key="slate_season")
    weeks = sorted(pd.to_numeric(frame[frame["Season"].astype(str) == str(selected_season)]["Week"], errors="coerce").dropna().astype(int).unique())
    default_week = _default_projection_week(schedule) if selected_season == season and not schedule.empty else (weeks[-1] if weeks else 1)
    week_index = weeks.index(default_week) if default_week in weeks else max(0, len(weeks) - 1)
    selected_week = st.selectbox("Week", weeks, index=week_index, key="slate_week") if weeks else 1
    view = frame[(frame["Season"].astype(str) == str(selected_season)) & (frame["Week"].astype(str) == str(selected_week))].copy()
...
def _render_setup() -> None:
    ...
    st.caption("No daily setup sequence and no API credential are required. Build and Slate automatically synchronize the schedule and available markets, refresh open-data ratings, simulate the current week, and settle completed results.")
...
def render() -> None:
    ...
    st.caption(f"{MODEL_VERSION} • Fully automatic spread, moneyline, and totals score distribution")
